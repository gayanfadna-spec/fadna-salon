...
const AgentDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (!storedAgent) {
            console.log("AgentDashboard: No user found, redirecting to login");
            navigate('/login');
            return;
        }
        ...
        const fetchData = async (agentId) => {
            try {
                // Fetch Orders
                const ordersRes = await axios.get(`${API_URL}/orders`, { params: { agentId } });
                if (ordersRes.data.success) setOrders(ordersRes.data.orders);

                // Fetch Stats
                const statsRes = await axios.get(`${API_URL}/analytics/agent-performance`, { params: { agentId } });
                if (statsRes.data.success && statsRes.data.stats.length > 0) {
                    setStats(statsRes.data.stats[0]);
                }

                // Fetch Item Performance
                const itemRes = await axios.get(`${API_URL}/analytics/item-performance`, { params: { agentId } });
        ...
        <div className="container animate-fade-in">
            <header className="admin-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                        <h1 style={{ margin: 0 }}>{agent.name} Dashboard</h1>
